...
nodes = [
	{ :hostname => 'appserver', :ip => '192.168.0.42' },
	{ :hostname => 'dbserver', :ip => '192.168.0.43' }
]
...
Vagrant.configure("2") do |config|
  ...
  nodes.each do |node|
	config.vm.define node[:hostname] do |nodeconfig|
		nodeconfig.vm.hostname = node[:hostname] + ".bth.se"
		nodeconfig.vm.network :private_network, ip: node[:ip]
	end
  end

  # Configure web server
  config.vm.define "web.bth.se" do |web|
	web.vm.hostname = "web"
	web.vm.network "forwarded_port", guest: 80, host: 8080
	web.vm.network :private_network, ip: '192.168.0.44'
  end

  # Configure 3 test machines
  (0..2).each do |i|
	config.vm.define "tst#{i}.bth.se" do |tst|
